...
// Normalize legacy LMP values (MM/DD/YYYY) into Y-m-d for the date input
$lmp_value = '';
$lmp_raw = trim((string) $a->lmp);
if ($lmp_raw !== '') {
    $lmp_ts = strtotime($lmp_raw);
    $lmp_value = $lmp_ts ? date('Y-m-d', $lmp_ts) : '';
}
$visit_ts = $a->visit_date ? strtotime($a->visit_date) : false;
$is_referral = (int) $a->referral_status === 1;

// Show the obstetric panel for female patients, or when this visit already has obstetric data
$has_ob_data = $lmp_raw !== '' || trim((string) $a->date_of_delivery) !== ''
    || (int) $a->gravida || (int) $a->parity || (int) $a->term || (int) $a->preterm
    || (int) $a->abortion || (int) $a->living;
$show_ob = !$is_male || $has_ob_data;
?>

<div class="appointment-wrapper">

<!-- Hero: whose appointment is being edited -->
<div class="ap-hero">
    <div class="hero-inner">
        <div class="hero-identity">
            <div class="hero-avatar"><?= htmlentities($initials ?: '?'); ?></div>
            <div>
                <div class="hero-eyebrow">Edit Appointment</div>
                ...
                <div class="hero-chips">
                    ...
                    <?php if($is_referral): ?><span class="hero-chip"><i class="ph ph-handshake"></i>Referral</span><?php endif; ?>
...
<?= form_open('Pages/appointment_edit/' . (int) $p->id . '/' . (int) $a->id, array('class' => 'parsley-examples', 'autocomplete' => 'off')); ?>
<input type="hidden" name="p_id" value="<?= (int) $p->id; ?>">
<input type="hidden" name="a_id" value="<?= (int) $a->id; ?>">
<input type="hidden" name="age" value="<?= $age; ?>">
<input id="cyVal" type="hidden" value="28">
<input id="weekVal" type="hidden" name="no_of_weeks" value="<?= (int) $a->no_of_weeks; ?>">
<input id="dayVal" type="hidden" name="no_of_days" value="<?= (int) $a->no_of_days; ?>">

<!-- Visit details -->
<div class="card form-card">
    <div class="card-header">
        <h5><span class="section-icon"><i class="ph ph-calendar-plus"></i></span>Visit Details</h5>
        <span class="header-note">Appointment #<?= (int) $a->id; ?></span>
    </div>
    <div class="card-body">
        <div class="form-row">
            <div class="form-group col-md-4">
                <label class="required">Date of Visit</label>
                <input type="date" required class="form-control" name="visit_date" value="<?= $visit_ts ? date('Y-m-d', $visit_ts) : date('Y-m-d'); ?>" max="<?= date('Y-m-d'); ?>" />
            </div>
            <div class="form-group col-md-4">
                <label class="required">Blood Pressure</label>
                <div class="input-unit">
                    <input type="text" required class="form-control" name="bp" value="<?= htmlentities((string) $a->bp); ?>" placeholder="120/80" pattern="\s*\d{2,3}\s*/\s*\d{2,3}\s*" title="Format: systolic/diastolic, e.g. 120/80" />
            ...
            <div class="form-group col-md-4">
                ...
                <div class="input-unit">
                    <input type="number" required class="form-control" name="weight" value="<?= htmlentities((string) $a->weight); ?>" placeholder="60" step="0.1" min="0" inputmode="decimal" />
        ...
        <div class="form-row">
            <div class="form-group col-md-4">
                ...
                <div class="segmented">
                    <input type="radio" onclick="refer()" id="customRadio1" name="ref" value="0" <?= !$is_referral ? 'checked' : ''; ?> />
                    <label for="customRadio1"><i class="ph ph-footprints"></i>Walk In</label>
                    <input type="radio" onclick="referral()" id="customRadio2" name="ref" value="1" <?= $is_referral ? 'checked' : ''; ?> />
                    <label for="customRadio2"><i class="ph ph-handshake"></i>Referral</label>
                </div>
            </div>
            <div class="form-group col-md-8">
                <label>Referral Company</label>
                <select class="form-control" id="company" name="ref_id" <?= !$is_referral ? 'disabled' : ''; ?> style="display:<?= $is_referral ? 'block' : 'none'; ?>;">
                    <option value="">Select referral company</option>
                    <?php foreach($patient as $row): ?>
                        <option value="<?= (int) $row->id; ?>" <?= (int) $a->referral_id === (int) $row->id ? 'selected' : ''; ?>><?= htmlentities($row->company); ?></option>
                    <?php endforeach; ?>
                </select>
                <div id="companyPlaceholder" class="form-control" style="display:<?= $is_referral ? 'none' : 'flex'; ?>; align-items:center; color:#a5b3c2; background:#f7fafd;">Only needed for referrals</div>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group col-md-12">
                ...
                <textarea class="form-control" rows="4" id="example-textarea" name="transaction" placeholder="Reason for the visit, procedures done, or anything the doctor should see…"><?= htmlentities((string) $a->transaction); ?></textarea>
            </div>
        </div>
    </div>
</div>
...
<div class="card form-card">
    <div class="card-body" style="padding: 16px 24px 24px;">
        <div class="ob-toggle" id="obToggle" onclick="toggleOb()" aria-expanded="<?= $has_ob_data ? 'true' : 'false'; ?>" aria-controls="pregnancySection" role="button" tabindex="0">
            <div class="ob-icon"><i class="ph ph-baby"></i></div>
            <div class="ob-text">
                <div class="ob-title">Obstetric Information</div>
                <div class="ob-desc">LMP, estimated delivery date and G/P/T/P/A/L. <?= $has_ob_data ? 'Expanded — this visit has obstetric data.' : 'Optional — expand for prenatal visits.'; ?></div>
            </div>
            <span class="ob-caret"><span class="ob-caret-text"><?= $has_ob_data ? 'Hide' : 'Show'; ?></span><i class="ph ph-caret-down"></i></span>
        </div>
        <div id="pregnancySection" style="display: <?= $has_ob_data ? 'block' : 'none'; ?>;">
            <div class="ob-panel">
                <div class="form-row">
                    <div class="form-group col-md-4">
                        ...
                        <input type="date" class="form-control" name="lmp" id="datepic1" value="<?= $lmp_value; ?>" max="<?= date('Y-m-d'); ?>">
                    </div>
                    <div class="form-group col-md-4">
                        <label>Estimated Date of Delivery</label>
                        <input type="text" id="esDate" name="date_of_delivery" class="form-control" value="<?= htmlentities((string) $a->date_of_delivery); ?>" readonly placeholder="Calculated from LMP" />
                    </div>
                    <div class="form-group col-md-4">
                        <label>&nbsp;</label>
                ...
                <div class="ob-grid">
                    <div class="form-group"><label>G <small>Gravida</small></label><input type="number" min="0" name="gravida" class="form-control" value="<?= (int) $a->gravida; ?>" /></div>
                    <div class="form-group"><label>P <small>Parity</small></label><input type="number" min="0" name="parity" class="form-control" value="<?= (int) $a->parity; ?>" /></div>
                    <div class="form-group"><label>T <small>Term</small></label><input type="number" min="0" name="term" class="form-control" value="<?= (int) $a->term; ?>" /></div>
                    <div class="form-group"><label>P <small>Preterm</small></label><input type="number" min="0" name="preterm" class="form-control" value="<?= (int) $a->preterm; ?>" /></div>
                    <div class="form-group"><label>A <small>Abortion</small></label><input type="number" min="0" name="abortion" class="form-control" value="<?= (int) $a->abortion; ?>" /></div>
                    <div class="form-group"><label>L <small>Living</small></label><input type="number" min="0" name="living" class="form-control" value="<?= (int) $a->living; ?>" /></div>
                </div>
            </div>
        </div>
    </div>
...
<div class="card form-card">
    <div class="card-header">
        <h5><span class="section-icon"><i class="ph ph-clipboard-text"></i></span>Recent Visits <span class="count-badge"><?= count($history); ?></span></h5>
    ...
    <div class="card-body" style="padding: 0;">
        ...
        <div class="empty-state">
            ...
            <p>No previous visits on record for this patient.</p>
        </div>
        <?php endif; ?>
    </div>
</div>
...
<?php if($show_ob): ?>
// Clear only the obstetric fields
function clearOb() {
