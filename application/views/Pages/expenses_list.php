<?php
$total_exp = is_array($data) ? count($data) : 0;
$total_amount = 0;
if (is_array($data)) {
    foreach ($data as $r) {
        $total_amount += (float) str_replace(',', '', (string) $r->amount);
    }
}
?>
<style>
...
.or-pill { font-family: 'DM Sans', monospace; font-size: 12.5px; font-weight: 600; color: #5a6b7d; background: #f1f5f9; padding: 3px 10px; border-radius: 6px; }
...
<div class="mhero">
    <div class="mhero-inner">
        <div>
            <h2><i class="ph ph-receipt"></i>Expenses</h2>
            <p>Track and manage clinic operating expenses</p>
        </div>
        <div class="hero-right">
            <div class="hero-stat">
                <i class="ph ph-files"></i>
                <div>
                    <div class="stat-count"><?= number_format($total_exp); ?></div>
            ...
            <div class="hero-stat">
                ...
                <div>
                    <div class="stat-count">&#8369;<?= number_format($total_amount, 2); ?></div>
                    <div class="stat-label">Total Spent</div>
                </div>
            </div>
            <a data-toggle="modal" href="#new" class="btn-hero"><i class="ph ph-plus"></i>New Expense</a>
        </div>
    </div>
</div>
...
<div class="card data-card">
    <div class="card-header">
        <h5><i class="ph ph-file-doc"></i>Expense Records <span class="count-badge"><?= number_format($total_exp); ?></span></h5>
        <span class="hdr-meta">Sorted by date, newest first</span>
    </div>
    <div class="card-body">
        <?php if(empty($data)): ?>
            <div class="empty-state">
                ...
                <span>Record your first expense using the button above.</span>
            </div>
        <?php else: ?>
        <div class="table-responsive">
            <table id="datatable" class="table table-modern">
                ...
                <tbody>
                    ...
                    ?>
                    <tr>
                        <td data-order="<?= $ts !== false ? $ts : 0; ?>"><span class="cell-date"><?= $ts !== false ? date('M d, Y', $ts) : htmlentities($row->date); ?></span></td>
                        ...
                        <td class="actions-cell">
                            <a data-toggle="modal" href="#edit<?= $row->id; ?>" class="act act-edit" title="Edit expense"><i class="ph ph-pencil-simple"></i></a>
                            <a href="<?= base_url(); ?>Pages/expenses_delete/<?= $row->id; ?>" class="act act-del" title="Delete expense" data-toggle="tooltip" onclick="return confirm('Delete this expense record? This cannot be undone.')"><i class="ph ph-trash"></i></a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="ph ph-pencil-simple"></i>Edit Expense</h5>
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label>Date</label>
                    <input type="date" name="date" value="<?= htmlentities($row->date, ENT_QUOTES); ?>" required class="form-control">
                </div>
                <div class="form-group">
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" name="or_no" value="<?= htmlentities($row->or_no, ENT_QUOTES); ?>" class="form-control">
                    </div>
                    <div class="form-group col-md-6">
                        <label>Amount</label>
                        <div class="input-money"><span class="peso">&#8369;</span>
                            <input type="number" step="0.01" min="0" name="amount" value="<?= htmlentities($row->amount, ENT_QUOTES); ?>" required class="form-control">
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button type="submit" name="edit" class="btn-m save"><i class="ph ph-check"></i>Update Expense</button>
            </div>
            </form>
        </div>
    </div>
...
<div id="new" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="ph ph-receipt"></i>New Expense</h5>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            </div>
            <?= form_open('Pages/expenses_list'); ?>
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label>Description</label>
                    <input type="text" name="description" required class="form-control" placeholder="e.g. Medical supplies" autofocus>
                </div>
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <label>Official Receipt No.</label>
                        <input type="text" name="or_no" class="form-control" placeholder="Optional">
                    </div>
                    <div class="form-group col-md-6">
                        <label>Amount</label>
                        <div class="input-money"><span class="peso">&#8369;</span>
                            <input type="number" step="0.01" min="0" name="amount" required class="form-control" placeholder="0.00">
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn-m cancel" data-dismiss="modal"><i class="ph ph-x"></i>Cancel</button>
                <button type="submit" name="add" class="btn-m save"><i class="ph ph-plus"></i>Add Expense</button>
            </div>
            </form>
        </div>
    </div>
