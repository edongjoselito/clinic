...
$totalDiscount = 0;
foreach ($rows as $r) {
    $totalAmount += (float) $r->amount_due;
...
<div class="mhero">
    <div class="mhero-inner">
        <div>
            <h2><i class="ph ph-cash-register"></i>Sales Summary</h2>
            <p>Revenue report by date range</p>
        </div>
        <div class="hero-right">
            ...
            <div class="hero-stat">
                ...
                <div>
                    <div class="stat-count">&#8369;<?= number_format($totalAmount, 2); ?></div>
...
<div class="card data-card">
    <div class="card-header">
        <h5><i class="ph ph-file-doc"></i>Sales Report <?= $filtered ? '<span class="count-badge">' . number_format(count($rows)) . '</span>' : ''; ?></h5>
        <?php if($filtered): ?>
    ...
    <div class="card-body">
        <?php if(!$filtered): ?>
            <div class="empty-state">
                ...
                <span>Choose a date range to see completed sales.</span><br>
        ...
        <?php elseif(empty($rows)): ?>
            <div class="empty-state">
                ...
                <span>No completed transactions between <?= date('M d', strtotime($df)); ?> and <?= date('M d, Y', strtotime($dt)); ?>.</span><br>
            ...
            <table class="table table-modern">
                ...
                <tbody>
                    <?php foreach($rows as $row):
                        $pname = trim($row->first_name . ' ' . $row->middle_name . ' ' . $row->last_name);
                        $ts = strtotime($row->date);
                    ?>
                    <tr>
                        <td><div class="cell-main"><?= $pname !== '' ? htmlentities(strtoupper($pname)) : '<span class="cell-sub">Walk-in</span>'; ?></div></td>
                        ...
                        <td class="text-right text-money">&#8369;<?= number_format((float) $row->total_retail, 2); ?></td>
                        <td class="text-right text-money text-neg"><?= (float) $row->discount > 0 ? '&minus;&#8369;' . number_format((float) $row->discount, 2) : '—'; ?></td>
                        <td class="text-right text-money">&#8369;<?= number_format((float) $row->amount_due, 2); ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
                <tfoot>
                    <tr>
                        <td colspan="3">Totals &middot; <?= number_format(count($rows)); ?> transaction<?= count($rows) !== 1 ? 's' : ''; ?></td>
                        <td class="text-right text-money">&#8369;<?= number_format($totalAmount + $totalDiscount, 2); ?></td>
                        <td class="text-right text-money text-neg">&minus;&#8369;<?= number_format($totalDiscount, 2); ?></td>
                        <td class="text-right text-money">&#8369;<?= number_format($totalAmount, 2); ?></td>
                    </tr>
                </tfoot>
            </table>
        </div>
