<?php
$total_users = is_array($data) ? count($data) : 0;
$me = (int) $this->session->id;
?>
<style>
...
.u-avatar { width: 38px; height: 38px; border-radius: 50%; background: linear-gradient(135deg, #1e88e5, #0d47a1); color: #fff; font-weight: 700; font-size: 14px; display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; }
.you-pill { font-size: 10px; font-weight: 700; color: #1565c0; background: #e8f4fd; padding: 1px 7px; border-radius: 10px; margin-left: 6px; text-transform: uppercase; letter-spacing: .4px; }
...
.tag.amber { background: #fff5e6; color: #d68910; }
...
.act-key { background: #fff5e6; color: #d68910; border-color: #f5e0b8; }
.act-key:hover { background: #f39c12; color: #fff; }
...
.pw-user { display: flex; align-items: center; gap: 10px; background: #f8fafc; border: 1px solid #eef2f6; border-radius: 10px; padding: 10px 14px; margin-bottom: 16px; font-size: 13.5px; color: #3d4f63; }
.pw-user i { color: #1e88e5; font-size: 18px; }
</style>
...
<div class="mhero">
    <div class="mhero-inner">
        <div>
            <h2><i class="ph ph-users-three"></i>User Accounts</h2>
            <p>Staff logins, roles and system access</p>
        </div>
        <div class="hero-right">
            <div class="hero-stat">
                <i class="ph ph-users-four"></i>
                <div>
                    <div class="stat-count"><?= number_format($total_users); ?></div>
                    <div class="stat-label">Accounts</div>
                </div>
            </div>
            <a href="<?= base_url(); ?>Pages/user_add" class="btn-hero"><i class="ph ph-user-circle-plus"></i>New User</a>
        </div>
    </div>
</div>
...
<div class="card data-card">
    <div class="card-header">
        <h5><i class="ph ph-identification-card"></i>System Users <span class="count-badge"><?= number_format($total_users); ?></span></h5>
        <span class="hdr-meta"><?= !empty($is_superadmin) ? 'All clinics' : 'This clinic only'; ?></span>
    </div>
    <div class="card-body">
        <?php if(empty($data)): ?>
            <div class="empty-state">
                ...
                <span>Create the first account using the New User button above.</span>
            </div>
        <?php else: ?>
            ...
            <table id="datatable" class="table table-modern">
                <thead>
                    <tr>
                        ...
                        <th>Position</th>
                        <th>Specialty</th>
                        <?php if(!empty($is_superadmin)): ?><th>Clinic</th><?php endif; ?>
                        <th class="text-right">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach($data as $row):
                        $full = trim(implode(' ', array_filter(array($row->first_name, $row->middle_name, $row->last_name))));
                        $initials = strtoupper(substr($row->first_name !== '' ? $row->first_name : $row->username, 0, 1) . substr($row->last_name !== '' ? $row->last_name : '', 0, 1));
                        $is_self = ((int) $row->id === $me);
                    ?>
                    <tr>
                        <td>
                            <div class="user-cell">
                                <span class="u-avatar"><?= htmlentities($initials !== '' ? $initials : substr($row->username, 0, 1)); ?></span>
                                <div>
                                    <div class="cell-main">
                                        <?= $full !== '' ? htmlentities($full) : htmlentities($row->username); ?>
                                        <?php if($is_self): ?><span class="you-pill">You</span><?php endif; ?>
                                    </div>
                                    <div class="cell-sub">@<?= htmlentities($row->username); ?></div>
                                </div>
                            </div>
                        </td>
                        <td><span class="tag grey"><?= htmlentities(ucfirst($row->position)); ?></span></td>
                        <td>
                            <?php if(!empty($row->specialty_name)): ?>
                                <span class="tag blue"><?= htmlentities($row->specialty_name); ?></span>
                            <?php else: ?>
                        ...
                        <td>
                            <?php if($row->is_superadmin): ?>
                                <span class="tag amber"><i class="ph ph-crown-simple"></i>Superadmin</span>
                            <?php elseif(!empty($row->clinic_name)): ?>
                                <?= htmlentities($row->clinic_name); ?>
                            <?php else: ?>
                        ...
                        <td class="actions-cell">
                            <a href="<?= base_url(); ?>Pages/user_update/<?= $row->id; ?>" class="act act-edit" title="Edit account" data-toggle="tooltip"><i class="ph ph-pencil-simple"></i></a>
                            <a data-toggle="modal" href="#pw<?= $row->id; ?>" class="act act-key" title="Reset password"><i class="ph ph-key"></i></a>
                            <?php if(!$is_self): ?>
                            <a href="<?= base_url(); ?>Pages/user_delete/<?= $row->id; ?>" class="act act-del" title="Delete account" data-toggle="tooltip" onclick="return confirm('Delete the account <?= htmlentities($row->username, ENT_QUOTES); ?>? This cannot be undone.')"><i class="ph ph-trash"></i></a>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
...
<?php foreach($data as $row): ?>
<div id="pw<?= $row->id; ?>" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="ph ph-key"></i>Reset Password</h5>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            </div>
            <?= form_open('Pages/users_list'); ?>
            <div class="modal-body">
                <input type="hidden" name="id" value="<?= (int) $row->id; ?>">
                <div class="pw-user"><i class="ph ph-user-circle"></i>Resetting password for <strong>@<?= htmlentities($row->username); ?></strong></div>
                <div class="form-group">
                    <label>New Password</label>
                    <input type="password" name="password" required class="form-control" placeholder="Enter new password" autocomplete="new-password" autofocus>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn-m cancel" data-dismiss="modal"><i class="ph ph-x"></i>Cancel</button>
                <button type="submit" name="edit" class="btn-m save"><i class="ph ph-lock-key"></i>Update Password</button>
            </div>
            </form>
        </div>
    </div>
